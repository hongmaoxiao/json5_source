...
JSON5.stringify = function(obj, replacer, space) {
  ...
  if (space) {
    throw new Error('JSON5 does not support the space parameter. Use JSON instead.');
  }
  ...
  function makeIndent(str, num, noNewLine) {
    if (num === 0 || !indent) {
      return "";
		...
		// no more than 10 spaces per indent
    num = Math.min(num, 10);
        ...
        if (obj_part === null) {
          ...
          for (var prop in obj_part) {
            if (obj_part.hasOwnProperty(prop)) {
              ...
              if (obj_part[prop] != undefined && value !== null) {
                var key = isWord(prop) ? prop : '"' + prop + '"';
                buffer += key + ':' + value + ',';
              }
            }
          }
